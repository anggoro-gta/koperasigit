                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <input type="checkbox" class="pinjaman" name="tcp_id[]" class="flat"
                                value="<?= $p->id ?>" />
                            <input type="hidden" name="fk_anggota_id[<?= $p->id ?>]" value="<?= $p->fk_anggota_id ?>">
                            <input type="hidden" name="fk_pinjaman_id[<?= $p->id ?>]" value="<?= $p->id ?>">
                            <input type="hidden" name="angsuran_ke[<?= $p->id ?>]" value="<?= $p->angsuran_ke ?>">
                            <input type="hidden" name="tenor[<?= $p->id ?>]" value="<?= $p->tenor ?>">
                            <input type="hidden" name="pokok[<?= $p->id ?>]" value="<?= number_format($p->pokok)  ?>">
                            <input type="hidden" name="tapim[<?= $p->id ?>]" value="<?= number_format($p->tapim)  ?>">
                            <input type="hidden" name="bunga[<?= $p->id ?>]" value="<?= number_format($p->bunga)  ?>">
                            <input type="hidden" name="jml_tagihan[<?= $p->id ?>]"
                                value="<?= number_format($p->jml_tagihan)  ?>">
                        </td>
                        <?php } ?>
                        <td scope="row"><?= ++$key ?></td>
                        <td><?= $p->tgl ?></td>
                        <td><?= $p->nama ?>
                            <?php
                                ...
                                if (!empty($p->is_anggota_baru)) {
                                    $badges[] = '<span class="label label-success">Baru</span>';
                                }

                                if (!empty($p->is_tunggakan)) {
                                    $badges[] = '<span class="label label-danger">Tunggakan</span>';
                                }

                                if (!empty($badges)) {
                                    echo '<br>' . implode(' ', $badges);
                                }
                            ?>
                        </td>
                        <td><?= $p->nip ?></td>
                        ...
                        <td>
                            <div class="btn-group text-center"><a class="btn btn-xs btn-danger"
                                    onclick="return confirm('Apakah Anda akan menghapus data?');"
                                    href="<?php echo base_url() ?>/Tagihan/delete_pinjaman/<?= $p->id ?>/<?= $p->fk_tagihan_id ?>"><i
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if ($readonly == false) { ?>
                            <input type="text" name="sukarela[<?= $value->id ?>]"
                                class="form-control col-md-7 col-xs-12 nominal" value="">
                            <?php } else { ?>
                        ...
                        <td>
                            <div class="btn-group text-center"><a class="btn btn-xs btn-danger"
                                    onclick="return confirm('Apakah Anda akan menghapus data?');"
                                    href="<?php echo base_url() ?>/Tagihan/delete_simpanan/<?= $value->id ?>/<?= $value->fk_tagihan_id ?>"><i
                                        class="fa fa-trash"></i></a></div>
                        </td>
                        <?php } ?>
                    </tr>
